import Image from 'vinisto_api_client/src/domain/image';
...
const getBundleImage = (
	...
	if (onlyIsMain)
		return mainImage
			? // @ts-expect-error TODO fix image typing
	...
	if (images.length > 0)
		return (
			// @ts-expect-error TODO fix image typing
			images[reverse ? images.length - 1 : 0].domainUrls?.[imageSize] ?? ''
		);

	return defaultImageUrls[imageSize] ?? '';
};

export * from './constants';

export default getBundleImage;

getBundleImage([], 'thumb_330x260');
