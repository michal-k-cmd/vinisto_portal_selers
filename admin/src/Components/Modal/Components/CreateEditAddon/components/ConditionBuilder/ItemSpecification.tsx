...
const ItemSpecification = ({
	...
	const filters: SpecificationFilterValue[] = [];

	const specificationDefinitionId =
		typeof initialConditions?.itemSpecificationConditionRequest
			?.specificationId === 'string'
			? initialConditions.itemSpecificationConditionRequest.specificationId
			: '';
	const specificationValues =
		// @ts-expect-error This is not in generated types, but it's in the API response
		initialConditions?.itemSpecificationConditionRequest?.allowedValues ?? [];
	if (specificationDefinitionId && specificationValues.length > 0) {
		...
		filters.push({
			...
			unit: specification?.unit
				...
				: undefined,
			values: specificationValues.map((val: string) => {
				if (
				...
				if (specification?.specificationType === SpecificationType.CHECK_BOX) {
					return {
						value: val,
