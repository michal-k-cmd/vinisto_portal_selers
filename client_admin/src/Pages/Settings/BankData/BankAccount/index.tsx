import { FC, useCallback, useContext } from 'react';
import { SupplierDataAction } from 'Pages/Settings/SupplierDataContext/constants';
import { SupplierDataContext } from 'Pages/Settings/SupplierDataContext';
import withForm from 'Components/SingleEditForm/withForm';

import { FIELD_NAME } from './constants';
import { BankAccountFormFields } from './interfaces';
import BankAccountInput from './input';

const FormBankAccount = withForm<BankAccountFormFields>(BankAccountInput);

const BankAccountForm: FC = () => {
	const { dispatch, bankAccountNumber } = useContext(SupplierDataContext);

	const handleOnSubmit = useCallback(
		async (values: BankAccountFormFields) => {
			const submitError = await dispatch([
				SupplierDataAction.setBankAccountNumber,
				values[FIELD_NAME],
			]);
			return Promise.resolve({
				[FIELD_NAME]: submitError,
			});
		},
		[dispatch]
	);

	return (
		<FormBankAccount
			formKey="bankAccount"
			onSubmit={handleOnSubmit}
			initialValues={{ [FIELD_NAME]: bankAccountNumber }}
			btnsClassName="single-edit-form__inline-btn-wrapper"
		/>
	);
};

export default BankAccountForm;
