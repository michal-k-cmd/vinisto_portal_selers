...
> {
	...
	constructor(props: InputSpinnerProps) {
		...
		const spinnerStep = this.getSpinnerStep(this.props.step);
		this.state = {
		...
		if (this.isTypeDecimal()) {
			if (this.realMatch('' + currentValue)) {
				...
				if (Number(currentValue) === this.state.min && event === 'none') {
					if (
						Number(currentValue) !== Number(this.state.lastEmittedValue) &&
						this.props.onChange
					) {
				...
				if (event === 'blur') this.setState({ value: currentValue });

				if (
					currentValue !== Number(this.state.lastEmittedValue) &&
					this.props.onChange
				) {
	...
	onBlur(): void {
		const currentValue = this.getValue(undefined);
		if (currentValue === '') {
			return;
		}
		if (this.isTypeDecimal()) {
	...
	getValue(num: any) {
		...
		if (typeof value == 'number') {
			...
			if (this.realMatch(value)) {
				if (
					this.countDecimalsFromString(value) === this.props.precision &&
					value.endsWith('0')
				) {
					value = Number(value);
				}
	...
	isDisabledButtonLeft(): boolean {
		return (
			this.props.disabled || this.minReached(this.parseNum(this.state.value))
		);
	}

	isDisabledButtonRight() {
		return (
			this.props.disabled || this.maxReached(this.parseNum(this.state.value))
		);
	}
